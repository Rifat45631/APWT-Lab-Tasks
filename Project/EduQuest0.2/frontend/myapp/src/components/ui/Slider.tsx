...
const Slider = ({
  ...
  return (
    <div className={`w-full ${className}`} {...props}>
      <div className="flex flex-row gap-x-12 justify-center overflow-hidden">
        {slides.slice(currentSlide * itemsPerPage, currentSlide * itemsPerPage + itemsPerPage).map((testimonial, index) => (
          <div
            key={testimonial.id || index}
            className="flex flex-col bg-global-7 rounded-[10px] w-full max-w-[400px] h-[auto] p-[30px] border-2 border-solid border-gray-300 transform transition-transform duration-500 ease-in-out"
          >
            ...
            <p className="font-rubik font-normal text-base leading-[30px] text-global-1 overflow-hidden text-ellipsis">
      ...
      <div className="flex justify-center gap-x-3 mt-6">
        {Array.from({ length: totalPages }).map((_, i) => (
          <div
            key={i}
            className={`w-5 h-2 rounded-full transition-all duration-300 transform ${
              i === currentSlide ? 'bg-global-3 scale-125' : 'bg-gray-300'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex justify-center gap-x-4 mt-6">
        <button
          onClick={goPrev}
          className="w-[60px] h-[60px] bg-global-3 border-2 border-solid border-global-3 rounded-full flex items-center justify-center shadow-[0px_14px_44px_#0e132214] hover:bg-red-600 hover:scale-110 transition-all"
        >
          <Image
